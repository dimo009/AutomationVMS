...
namespace Vulnerabilities.Web.Controllers
{
    ...
    public class ServerController : Controller
    {
        ...
        public IActionResult ImpactedServers(int id, string searchBy, string search)
        {
            if (searchBy == "System name")
            {
                return View(new VServersListingViewModel
                {
                    ServersAffectedByOneVulnerability = this.servers.AllServersAffectedByOneVulnerability(id).Where(s => s.SystemName.StartsWith(search) || search == null)
            ...
            else
            {
                return View(new VServersListingViewModel
                {
                    ServersAffectedByOneVulnerability = this.servers.AllServersAffectedByOneVulnerability(id).Where(s => s.TechnicalOwner.StartsWith(search) || search == null)
